...
# Reading files, string manipulation, and factors
...
Factors can be placed in reverse order as well.
...
Factor columns store both the actual values as well as the set of possible values, so dropping a level from a factor column is not as easy as excluding rows with factor values one does not want.
