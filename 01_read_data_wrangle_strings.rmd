...
Instead of using `stringr`, we will use the `stringi` package on which it is based, and read files using the `data.table` package. In place of `glue`, we will use the `base` function `sprintf`.
...
`stri_which` has the same logic except that it returns the vector position and not the elements.
...
stri_match(string = c("1970-somemonth-01",
...
stri_match_all(string = c("1970-somemonth-01 or maybe 1990/anothermonth/01"),
              pattern = "(\\d{4})[\\-\\/]([a-z]+)")
...
stri_extract(string = c("1970-somemonth-01 or maybe 1990/anothermonth/01",
                       "1990-somemonth-01 or maybe 2001/anothermonth/01"),
            pattern = "(\\d{4})[\\-\\/]([a-z]+)")

# extracting all full matches using stri_extract all
stri_extract_all(string = c("1970-somemonth-01 or maybe 1990/anothermonth/01",
                           "1990-somemonth-01 or maybe 2001/anothermonth/01"),
                pattern = "(\\d{4})[\\-\\/]([a-z]+)")
```
...
stri_split(string = c("1970-somemonth-01",
                     "1990/anothermonth/01"),
          pattern = "[\\-\\/]")
```
...
```{r}
# split on either a hyphen or a forward slash
stri_split_fixed(string = "dir_level_1/dir_level_2/file.ext",
                pattern = "/",
                n = 2)
```
...
stri_split(filename_without_spaces, " ")
```
...
stri_sub(string = c("1970-somemonth-01",
                   "1990-anothermonth-01",
                   "2010-thismonth-01"),
        start = 1, end = 4)
```
...
```{r}
stri_trunc(string = c("bananas are great and wonderful
...
- `stri_wrap` (of dubious use),

- `stri_interp`, `stri_glue*` (better to use `glue`; see below),

- `stri_sort`, `stri_order` (used in sorting a character vector),

- `stri_to_case*` (case conversion), and

- `stri_view*` (a graphical view of search pattern matches).
